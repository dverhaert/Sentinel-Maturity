...
| **Defender for Servers P2 benefit** | Pooled daily allowance of 500 MB × number of **Defender for Servers P2 licences** (one licence = one protected server, billed per hour — not the count of AMA-covered machines), applied to specific eligible security tables: SecurityAlert, SecurityBaseline, SecurityBaselineSummary, SecurityDetection, SecurityEvent, WindowsFirewall, ProtectionStatus, MDCFileIntegrityMonitoringEvents, the Microsoft-SecurityEvent stream within WindowsEvent, and Update / UpdateSummary (only when the Update Management solution isn't running in the workspace, or solution targeting is enabled). Individual servers can ingest more, as long as the pooled allowance isn’t exceeded. `LinuxAuditLog`, the general `Syslog` table, `CommonSecurityLog`, `W3CIISLog`, and non-SecurityEvent `WindowsEvent` channels (PowerShell, Sysmon, AppLocker) are **not** eligible. | [Data ingestion benefit](https://learn.microsoft.com/en-us/azure/defender-for-cloud/data-ingestion-benefit) |
