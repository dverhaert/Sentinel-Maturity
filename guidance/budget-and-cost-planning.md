...
| **Defender for Servers P2 benefit** | Pooled daily allowance of 500 MB × number of AMA-covered servers, applied to specific eligible security tables (SecurityEvent, WindowsEvent [Microsoft-SecurityEvent stream only], LinuxAuditLog, SecurityAlert, WindowsFirewall, ProtectionStatus, MDCFileIntegrityMonitoringEvents, SecurityBaseline / SecurityBaselineSummary / SecurityDetection, Update / UpdateSummary). Individual servers can ingest more, as long as the pooled allowance isn’t exceeded. `CommonSecurityLog`, `W3CIISLog`, the general `Syslog` table, and non-SecurityEvent `WindowsEvent` channels (PowerShell, Sysmon, AppLocker) are **not** eligible. | [Data ingestion benefit](https://learn.microsoft.com/en-us/azure/defender-for-cloud/data-ingestion-benefit) |
